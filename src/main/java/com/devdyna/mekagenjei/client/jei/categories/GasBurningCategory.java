...
public class GasBurningCategory<T> extends AbstractRecipeCategory<zStatic.GASBURNING> {
        ...
        public void draw(zStatic.GASBURNING recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics,
                ...
                var fuel = recipe.getGas().getData(IMekanismDataMapTypes.INSTANCE.chemicalFuel());
                var check = fuel == null;

                var time = check ? 0 : fuel.burnTicks();
                var rate = check ? 0 : Numbers.jouleToFE(fuel.energyDensity()) * (double) 256 / fuel.burnTicks();
                ...
                guiGraphics.drawString(font,
                                ...
                                0xFFFFFF);
                guiGraphics.drawString(font,
                                (Screen.hasShiftDown() ? Numbers.toCompact(rate) : Numbers.simplify(rate)) + " FE/tick",
                                46,
                                18,
                                0xFFFFFF);
                guiGraphics.drawString(font,
                                (Screen.hasShiftDown() ? Numbers.toCompact(total) : Numbers.simplify(total))
                                                + " FE",
                                46, 32,
                                0xFFFFFF);
                stack.popPose();
